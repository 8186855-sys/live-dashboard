import {
  ...
  getTimelineByDateWithTZ,
  getTimelineByDateAndDeviceWithTZ,
} from "../db";
...
export async function handleTimeline(url: URL): Promise<Response> {
  ...
  if (tzOffsetMinutes && !isNaN(tzOffsetMinutes) && Math.abs(tzOffsetMinutes) <= 840) {
    ...
    activities = deviceId
      ? await getTimelineByDateAndDeviceWithTZ(date, deviceId, modifier)
      : await getTimelineByDateWithTZ(date, modifier);
  } else {
    // No timezone offset — use UTC (backwards compatible)
    activities = deviceId
      ? await getTimelineByDateAndDevice(date, deviceId)
      : await getTimelineByDate(date);
  }
  ...
  for (let i = 0; i < activities.length; i++) {
    ...
    for (let j = i + 1; j < activities.length; j++) {
      if ((activities[j] as any).device_id === a.device_id) {
        endedAt = (activities[j] as any).started_at;
        break;
      }
    }
