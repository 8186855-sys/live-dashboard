...
import {
  insertManyHealthRecords,
  getHealthRecordsByDate,
  getHealthRecordsByDateAndDevice,
  getHealthRecordsByDateWithTZ,
  getHealthRecordsByDateAndDeviceWithTZ,
  getHealthRecordsByDateRange,
  getHealthRecordsByDateRangeAndDevice,
} from "../db";
...
export async function handleHealthData(req: Request): Promise<Response> {
  ...
  try {
    const inserted = await insertManyHealthRecords(toInsert);
  ...
  try {
    if (tzOffsetMinutes && !isNaN(tzOffsetMinutes) && Math.abs(tzOffsetMinutes) <= 840) {
      ...
      const records = deviceId
        ? await getHealthRecordsByDateAndDeviceWithTZ(date, deviceId, modifier)
        : await getHealthRecordsByDateWithTZ(date, modifier);
    ...
    const records = deviceId
      ? await getHealthRecordsByDateRangeAndDevice(startOfDay, startOfNextDay, deviceId)
      : await getHealthRecordsByDateRange(startOfDay, startOfNextDay);
